...
public class OwnerProfile : Profile
{
    public OwnerProfile()
    {
        ...
        CreateMap<OwnerRequest, OwnerRequestDto>()
            .ForMember(dest => dest.StatusId, opt => opt.MapFrom(src => src.StatusId)) // int → int
            ...
            .ForMember(dest => dest.ReviewedByName, opt => opt.MapFrom(src => 
                src.ReviewedByNavigation != null ? src.ReviewedByNavigation.FullName : null));
        // DTO → Entity (Create)
        ...
        CreateMap<ReviewOwnerRequestDto, OwnerRequest>()
            .ForMember(dest => dest.ReviewedAt, opt => opt.MapFrom(_ => DateTime.UtcNow))
